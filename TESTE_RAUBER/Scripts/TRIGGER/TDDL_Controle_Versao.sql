...
BEGIN	
	...
	,	@URL_PULL_REQUEST NVARCHAR(100) = 'https://dev.azure.com/tecbem/Transact-SQL/_git/sicred_matone_hml/pullrequest/'
	...
	IF(@MAX_DATA_ATUALIZACAO IS NOT NULL AND @DIFF_DIA_ATUAL < 30 --MINUTOS
		OR @ULTIMO_HASH = @CHECKSUM
	)
	BEGIN		
		IF(EXISTS(
			SELECT 1
			...
			AND USUARIO_ROBO_VERSIONAMENTO = @USUARIO_ATUALIZACAO
		))
		BEGIN
			...
			SET @MENSAGEM_URL_PULL_REQUEST = 'https://dev.azure.com/tecbem/Transact-SQL/_git/sicred_matone_hml/commits?itemVersion=GBmaster'
	...
	SET @PROCESSADO = IIF((@TIPO_EVENTO IN('DROP_VIEW', 'DROP_PROCEDURE', 'DROP_FUNCTION', 'DROP_TRIGGER')
						OR EXISTS(
							SELECT 1 
							FROM [TESTE_RAUBER_VERSIONAMENTO].[dbo].[BANCO_ORIGEM_PARA_BANCO_DESTINO]
							WHERE @NOME_DATABASE IN(NOME_BANCO_ORIGEM, NOME_BANCO_DESTINO)
	...
	IF(EXISTS(
		...
		BEGIN
			DECLARE 
				@ASSUNTO_EMAIL VARCHAR(100) = 'OBJETO COMPILADO EM PRODUÇÃO'
			,	@MENSAGEM_ERRO VARCHAR(700) = '<br /><br />'
			,	@DESTINATORIOS VARCHAR(100) = 'rrauber@bempromotora.com.br'		
